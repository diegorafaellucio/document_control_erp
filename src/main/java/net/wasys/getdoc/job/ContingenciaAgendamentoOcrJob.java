...
import net.wasys.util.ddd.TransactionRunnable;
 ...
 */
@Service
public class ContingenciaAgendamentoOcrJob extends TransactionWrapperJob {
	...
	public void execute() {
		...
		try {
			...
			filtro.setStatus(StatusLogOcr.PRE_AGENDADO);
			...
			systraceThread("agendando... " + ids.size() + ": " + ids);
	...
	private void processar(List<Long> ids) {
		...
		for (final Long logOcrId : ids) {
			...
			job.setRunnable(new TransactionRunnable() {
				public void run() throws Exception {
					logOcrService.agendarOcr(logOcrId);
				}
			});
			job.setExceptionHandler(new ExceptionHandler() {
				public void handle(Exception e, String stackTrace) {

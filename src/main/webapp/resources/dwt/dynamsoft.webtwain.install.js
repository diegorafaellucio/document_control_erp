...
function _show_install_dialog(ProductName, objInstallerUrl, bHTML5, iPlatform, bIE, bSafari, bSSL, strIEVersion) {
	...
	title = 'Por favor, instale o plugin.';
	...
	if (iPlatform == EnumDWT_PlatformType.enumLinux || navigator.userAgent.toLowerCase().indexOf("firefox") > -1) {
		...
		ObjString.push('<div><span class="dynamsoft-dwt-installdlg-text" style="right: 125px">Baixar</span>');
		ObjString.push('<span class="dynamsoft-dwt-installdlg-text" style="right: 6px">Instalar</span>');
		ObjString.push('<span class="dynamsoft-dwt-installdlg-text" style="left: 115px">Usar</span>');
		ObjString.push('</div>');
		ObjString.push('</div>');
	...
	if (bHTML5) {
		if (bIE) {
			...
			ObjString.push('<div class="dynamsoft-dwt-dlg-red">Após a instalação, por favor <strong>REINICIE</strong> seu navegador.</div>');
			ObjString.push('</div>');
		}
